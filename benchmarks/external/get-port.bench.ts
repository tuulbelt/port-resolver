...
/**
 * API-Equivalent Comparison: port-resolver vs get-port
 *
 * Compares equivalent operations between port-resolver and get-port (sindresorhus)
...
group('Single Port Allocation (API-Equivalent)', () => {
...
group('Port with Preferred Port (API-Equivalent)', () => {
...
group('Concurrent Allocation (API-Equivalent)', () => {
  ...
  bench('get-port: 5 parallel', async () => {
    const promises = Array.from({ length: 5 }, () => getPortLib());
    result = await Promise.all(promises);
  });
});

group('Concurrent Allocation - 10 Parallel (API-Equivalent)', () => {
  ...
  bench('get-port: 10 parallel', async () => {
...
console.log('  API-Equivalent Comparison: port-resolver vs get-port');
console.log('═'.repeat(70));
