 ...
 */
export async function findAvailablePort(config, exclude = new Set()) {
 ...
 */
function ensureRegistryDir(config) {
    const pathResult = validatePath(config.registryDir);
    if (!pathResult.ok) {
 ...
 */
function filterStaleEntries(entries, staleTimeout) {
    const now = Date.now();
    const active = [];
    const stale = [];
...
async function main() {
    ...
    if (args.registryDir !== undefined)
        config.registryDir = args.registryDir;
    if (args.allowPrivileged !== undefined)
        config.allowPrivileged = args.allowPrivileged;
